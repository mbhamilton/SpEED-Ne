function [mean_r_sqr_c_W_perm,mean_r_sqr_c_TH_perm,mean_r_sqr_delta_W_perm,mean_r_sqr_delta_TH_perm,...
    ...
    var_r_squared_c_W_perm,var_r_squared_c_TH_perm,var_r_squared_delta_W_perm,var_r_squared_delta_TH_perm] = ...
    permute_data_2(genotypes,n_permutations,num_individuals,num_loci,threshold_allele_freq,r_squared_c_W,r_squared_c_TH,r_squared_delta_W,r_squared_delta_TH,alpha_value,make_table,make_graphs,make_files)

    % Function to permute genotype among loci data based on Sved et al. (2013).
    %
    % version 1.1 July 18, 2017
    ...
    %
    %   var_r_squared_c_W_perm = variance in r_squared_c_W_permute over all permuted data sets
    %   var_r_squared_c_TH_perm = variance in r_squared_c_TH_permute over all permuted data sets
    %   var_r_squared_delta_W_perm = variance in r_squared_delta_W_permute over all permuted data sets
    %   var_r_squared_delta_TH_perm = variance in r_squared_delta_TH_permute over all permuted data sets
    ...
    if make_graphs == 1
        ...
        hist(r_squared_c_W_permute); 
        xlabel('r_c^2 AFW permute')
        ylabel('Count')

        subplot(2,2,2);
        hist(r_squared_c_TH_permute); 
        xlabel('r_c^2 AFT permute')
        ylabel('Count')

        subplot(2,2,3)
        hist(r_squared_delta_W_permute);
        xlabel('r_{\Delta}^2 AFW permute')
        ylabel('Count')

        subplot(2,2,4)
        hist(r_squared_delta_TH_permute); 
        xlabel('r_{\Delta}^2 AFT permute')
        ylabel('Count')

        suptitle('Distributions of r^2 values with permuted genotype data.');
        hold off;
    ...
    if make_table
        ...
        fprintf('---------------------\n');
        fprintf('permuted r^2_{comp} AFW:\n');
        fprintf('\tmean = %8.6g\n',mean_r_sqr_c_W_perm);
        fprintf('\tmedian = %8.6g\n\n',median_r_sqr_c_W_perm);

        Ne = 1/(3*(r_squared_c_W - mean_r_sqr_c_W_perm));
        if Ne >= 0
            fprintf('\tNe (mean permuted r^2 as correction factor) = %8.2f\n', Ne);
        else
        ...
        if Ne >= 0
            fprintf('\tNe (median permuted r^2 as correction factor) = %8.2f\n', Ne);
        else
        ...
        if upper_bound_Ne >= 0 
            fprintf('\tNe %g%% confidence intervals:\t%8.2f - %8.2f\n',(1-alpha_value)*100,lower_bound_Ne,upper_bound_Ne);
        else
            fprintf('\tNe %g%% confidence intervals:\t%8.2f - %s\n',(1-alpha_value)*100,lower_bound_Ne,'infinity');
        end

        fprintf('---------------------\n');

        fprintf('permuted r^2_{comp} AFT:\n');
        ...
        fprintf('permuted r^2_{delta} AFW:\n');
        ...
        fprintf('permuted r^2_{delta} AFT:\n');
    ...
    function [r_squared_c_W_permute,r_squared_c_TH_permute,r_squared_delta_W_permute,r_squared_delta_TH_permute,...
        ...
        % compute variances for permutations
        var_r_squared_c_W_perm = var(r_squared_c_W_permute);
        var_r_squared_c_TH_perm = var(r_squared_c_TH_permute);
        var_r_squared_delta_W_perm = var(r_squared_delta_W_permute);
        var_r_squared_delta_TH_perm = var(r_squared_delta_TH_permute);
