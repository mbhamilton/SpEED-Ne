function [jack_pct_CI_values,jack_norm_CI_values,chi_CI_values,effective_n_c,effective_n_delta] = wd_confidence_intervals_v3(num_loci,r_squared_c_values,r_squared_delta_values,c_locus_pairs,delta_locus_pairs,...
    ...
    [mean_rows, ~] = size(j_r_squared_delta);
    matrix_mean_r_delta_j = repmat(mean_r_delta_j,mean_rows,1);

    if ~isequal(size(j_r_squared_delta), size(matrix_mean_r_delta_j))
        fprintf('j_r_squared_delta and matrix_mean_r_delta_j are not the same size:\n');
        disp(size(j_r_squared_delta));
        disp(size(matrix_mean_r_delta_j));
    end
    ...
    if print_output
        ...
        fprintf('A value of n'' >> than number of locus pairs suggests violation of assumptions\n');
