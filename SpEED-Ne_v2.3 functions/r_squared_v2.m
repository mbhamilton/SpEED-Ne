function varargout = r_squared_v2(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @r_squared_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @r_squared_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before r_squared_v2 is made visible.
function r_squared_v2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to r_squared_v2 (see VARARGIN)

% Choose default command line output for r_squared_v2
...
% UIWAIT makes r_squared_v2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = r_squared_v2_OutputFcn(hObject, eventdata, handles) 
...
function run_button_Callback(hObject, eventdata, handles)
    ...
    fprintf('Weir (1979) and Waples and Do (2006).\n');
    fprintf('-------------------------------------------------------------------------------------------------\n');
    ...
    ub_r_squared_c_W = (har_mean_S_c/(har_mean_S_c - 1))*r_squared_c_W;
    ub_r_squared_delta_W = (har_mean_S_delta/(har_mean_S_delta - 1))*r_squared_delta_W;
    ...
    ub_r_squared_c_TH = (har_mean_S_c/(har_mean_S_c - 1))*r_squared_c_TH;
    ub_r_squared_delta_TH = (har_mean_S_delta/(har_mean_S_delta - 1))*r_squared_delta_TH;
    ...
    fprintf('=================================================================================================\n');
    fprintf('Estimates of Ne based on 1/(3 x r^2 - (r^2 correction factor)) without S/(S-1) bias correction.\n');
    fprintf('The r^2 correction factor is based on harmonic mean sample size S.\n');
    fprintf('-------------------------------------------------------------------------------------------------\n');
    fprintf('Ne = %5.2f based on r^2_{c} with AFW and harmonic mean number of individuals for all locus pairs.\n', 1/(3*(ub_r_squared_c_W - correction_factor_har_mean_S)) );
    fprintf('Ne = %5.2f based on r^2_{c} with AFT and harmonic mean number of individuals for all locus pairs.\n', 1/(3*(ub_r_squared_c_TH - correction_factor_har_mean_S)) );
    fprintf('\n');
    fprintf('Ne = %5.2f based on r^2_{delta} with AFW and harmonic mean number of individuals for all locus pairs.\n', 1/(3*(ub_r_squared_delta_W - r_delta_correction_factor_har_mean_S)) );
    fprintf('Ne = %5.2f based on r^2_{delta} with AFT and harmonic mean number of individuals for all locus pairs.\n', 1/(3*(ub_r_squared_delta_TH - r_delta_correction_factor_har_mean_S)) );
    fprintf('=================================================================================================\n');
    
    
    % Compute Waples (2006) statistical-fit corrected Ne estimate with each version of the sample size 
    % equations based on Table 1 in Waples and Do (2007). Utilizes unbiased estimates of r_squared delta. 
    fprintf('\n');
    fprintf('=================================================================================================\n');
    ...
    [waples_AFW_Ne] = Waples_Ne_estimate(har_mean_S_delta,r_squared_delta_W); % Waples Ne from r_squared_delta AFW
    fprintf('AFW Ne = %8.2f\n', waples_AFW_Ne);
    
    [waples_AFT_Ne] = Waples_Ne_estimate(har_mean_S_delta,r_squared_delta_TH); % Waples Ne from r_squared_delta AFT
    fprintf('AFT Ne = %8.2f\n', waples_AFT_Ne);
    
    [waples_ub_AFW_Ne] = Waples_Ne_estimate(har_mean_S_delta,ub_r_squared_delta_W); % Waples Ne from r_squared_delta_UB AFW
    fprintf('UB AFW Ne = %8.2f\n', waples_ub_AFW_Ne);
    
    [waples_ub_AFT_Ne] = Waples_Ne_estimate(har_mean_S_delta,ub_r_squared_delta_TH); % Waples Ne from r_squared_delta_UB AFT
    ...
    if permute == 1
        ...
        [~,~,~,~,median_r_sqr_c_AFW_perm,median_r_sqr_c_AFT_perm,~,~,~,~,~,~] = ...
            permute_data_2(genotypes,n_permutations,num_individuals,num_loci,threshold_allele_freq,r_squared_c_W,r_squared_c_TH,r_squared_delta_W,r_squared_delta_TH,alpha,make_table,make_graphs,make_files);
    ...
    if jack_individuals == 1
        ...
        if num_individuals > 2
            ...
            [~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~] = jackknife_individuals_v2(r_squared_c_W,r_squared_delta_W,r_squared_c_TH,r_squared_delta_TH,median_r_sqr_c_AFW_perm,median_r_sqr_c_AFT_perm,...
                r_delta_correction_factor_har_mean_S,genotypes,num_individuals,num_loci,threshold_allele_freq,alpha,print_output,make_graphs,make_files);   
