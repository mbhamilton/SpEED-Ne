function [jack_pct_CI_values,jack_norm_CI_values,chi_CI_values,effective_n_c,effective_n_delta] = wd_confidence_intervals_v2(num_loci,r_squared_c_values,r_squared_delta_values,c_locus_pairs,delta_locus_pairs,...
    r_squared_c_W,r_squared_c_TH,r_squared_delta_W,r_squared_delta_TH,correction_factor_har_mean_S,UB_weight_comp,UB_weight_delta,median_r_sqr_c_AFW_perm,median_r_sqr_c_AFT_perm,alpha,print_output,make_graphs)

% Function to compute confidence intervals for estimates of r^2 and Ne
% based on methods decribed by Waples (2006) and Waples & Do (2008). Also
% computes confidence intervals based on jackknifing over all locus pairs.
%
% version 2.1 21 March 2024
%
% Inputs:
...
%
%           1,9 - Ne from unbiased r_squared_c AFW lower bound CI
%           1,10 - Ne from unbiased r_squared_c AFW upper bound CI
%           1,11 - Ne from unbiased r_squared_c AFT lower bound CI
%           1,12 - Ne from unbiased r_squared_c AFT upper bound CI
%
%           1,13 - Ne from unbiased r_squared_delta AFW lower bound CI
%           1,14 - Ne from unbiased r_squared_delta AFW upper bound CI
%           1,15 - Ne from unbiased r_squared_delta AFT lower bound CI
%           1,16 - Ne from unbiased r_squared_delta AFT upper bound CI
%
...
%**************
    ...
    Ne_r_squared_c_AFW_UB = 1/(3*(r_squared_c_W*UB_weight_comp - median_r_sqr_c_AFW_perm));
    ...
    jack_pct_CI_values(1,9) = 1/(3*(j_r_squared_c_W_upper*UB_weight_comp - median_r_sqr_c_AFW_perm)); % Ne from r_squared_c AFW UB lower bound CI
    jack_pct_CI_values(1,10) = 1/(3*(j_r_squared_c_W_lower*UB_weight_comp - median_r_sqr_c_AFW_perm)); % Ne from r_squared_c AFW UB upper bound CI
    ...
    jack_norm_CI_values(1,9) = 1/(3*(r_squared_c_AFW_norm_CI(1,2)*UB_weight_comp - median_r_sqr_c_AFW_perm)); % Ne from r_squared_c AFW UB lower bound CI
    jack_norm_CI_values(1,10) = 1/(3*(r_squared_c_AFW_norm_CI(1,1)*UB_weight_comp - median_r_sqr_c_AFW_perm)); % Ne from r_squared_c AFW UB upper bound CI
    %-----------------

    % estimates for r^2_{comp} AFT
    Ne_r_squared_c_AFT = 1/(3*(r_squared_c_TH - median_r_sqr_c_AFT_perm));
    Ne_r_squared_c_AFT_UB = 1/(3*(r_squared_c_TH*UB_weight_comp - median_r_sqr_c_AFT_perm));
    ...
    jack_pct_CI_values(1,11) = 1/(3*(j_r_squared_c_TH_upper*UB_weight_comp - median_r_sqr_c_AFT_perm)); % Ne from r_squared_c AFW UB lower bound CI
    jack_pct_CI_values(1,12) = 1/(3*(j_r_squared_c_TH_lower*UB_weight_comp - median_r_sqr_c_AFT_perm)); % Ne from r_squared_c AFW UB upper bound CI
    ...
    jack_norm_CI_values(1,11) = 1/(3*(r_squared_c_AFT_norm_CI(1,2)*UB_weight_comp - median_r_sqr_c_AFT_perm)); % Ne from r_squared_c AFW UB lower bound CI
    jack_norm_CI_values(1,12) = 1/(3*(r_squared_c_AFT_norm_CI(1,1)*UB_weight_comp - median_r_sqr_c_AFT_perm)); % Ne from r_squared_c AFW UB upper bound CI
    %-----------------

    % estimates for r^2_{delta} AFW
    Ne_r_squared_delta_AFW = 1/(3*(r_squared_delta_W - correction_factor_har_mean_S));
    Ne_r_squared_delta_AFW_UB = 1/(3*(r_squared_delta_W*UB_weight_delta - correction_factor_har_mean_S));
    ...
    jack_pct_CI_values(1,13) = 1/(3*(j_r_squared_delta_W_upper*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_c AFW UB lower bound CI
    jack_pct_CI_values(1,14) = 1/(3*(j_r_squared_delta_W_lower*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_c AFW UB upper bound CI
    ...
    jack_norm_CI_values(1,13) = 1/(3*(r_squared_delta_AFW_norm_CI(1,2)*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_delta AFW UB upper bound CI
    jack_norm_CI_values(1,14) = 1/(3*(r_squared_delta_AFW_norm_CI(1,1)*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_delta AFW UB lower bound CI
    %-----------------

    % estimates for r^2_{delta} AFT
    Ne_r_squared_delta_AFT = 1/(3*(r_squared_delta_TH - correction_factor_har_mean_S));
    Ne_r_squared_delta_AFT_UB = 1/(3*(r_squared_delta_TH*UB_weight_delta - correction_factor_har_mean_S));
    ...
    jack_pct_CI_values(1,15) = 1/(3*(j_r_squared_delta_TH_upper*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_delta AFT UB lower bound CI
    jack_pct_CI_values(1,16) = 1/(3*(j_r_squared_delta_TH_lower*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_delta AFT UB upper bound CI    
    ...
    jack_norm_CI_values(1,15) = 1/(3*(r_squared_delta_AFT_norm_CI(1,2)*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_delta AFT UB upper bound CI
    jack_norm_CI_values(1,16) = 1/(3*(r_squared_delta_AFT_norm_CI(1,1)*UB_weight_delta - correction_factor_har_mean_S)); % Ne from r_squared_delta AFT UB lower bound CI
    %-----------------
    ...
    if print_output
        ...
        % estimates for r^2_{comp} AFW
        fprintf('unbiased r^2_{comp} AFW:\n');

        % percentile
        fprintf('  percentile CI: %8.6g\t%8.6f\t%8.6f\t%8.6g\n',r_squared_c_W*UB_weight_comp,j_r_squared_c_W_lower*UB_weight_comp,j_r_squared_c_W_upper*UB_weight_comp,c_locus_pairs);

        if jack_pct_CI_values(1,10) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_c_AFW_UB, jack_pct_CI_values(1,9), jack_pct_CI_values(1,10));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_c_AFW_UB, jack_pct_CI_values(1,9), 'infinity');
        end
        fprintf('\n');

        % normal distribution
        fprintf('  normal CI:\t%8.6g\t%8.6f\t%8.6f\t%8.6g\n',r_squared_c_W*UB_weight_comp,r_squared_c_AFW_norm_CI(1,1)*UB_weight_comp,r_squared_c_AFW_norm_CI(1,2)*UB_weight_comp,c_locus_pairs);

        if jack_norm_CI_values(1,10) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_c_AFW_UB, jack_norm_CI_values(1,9), jack_norm_CI_values(1,10));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_c_AFW_UB, jack_norm_CI_values(1,9), 'infinity');
        end
        fprintf('\n');

        %-----------------------------
        % estimates for r^2_{comp} AFT
        fprintf('unbiased r^2_{comp} AFT:\n');

        % percentile
        fprintf('  percentile CI: %8.6g\t%8.6f\t%8.6f\t%8.6g\n',r_squared_c_TH*UB_weight_comp,j_r_squared_c_TH_lower*UB_weight_comp,j_r_squared_c_TH_upper*UB_weight_comp,c_locus_pairs);

        % Ne_r_squared_c_TH_UB = 1/(3*(r_squared_c_TH*UB_weight_comp - median_r_sqr_c_AFT_perm)); % Ne estimated with UB r_squared_c AFW median permute

        if jack_pct_CI_values(1,12) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_c_AFT_UB, jack_pct_CI_values(1,11), jack_pct_CI_values(1,12));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_c_AFT_UB, jack_pct_CI_values(1,11), 'infinity');
        end
        fprintf('\n');

        % normal distribution
        fprintf('  normal CI:\t%8.6g\t%8.6f\t%8.6f\t%8.6g\n',r_squared_c_TH*UB_weight_comp,r_squared_c_AFT_norm_CI(1,1)*UB_weight_comp,r_squared_c_AFT_norm_CI(1,2)*UB_weight_comp,c_locus_pairs);

        if jack_norm_CI_values(1,12) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_c_AFT_UB, jack_norm_CI_values(1,11), jack_norm_CI_values(1,12));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_c_AFT_UB, jack_norm_CI_values(1,11), 'infinity');
        end
        fprintf('\n');

        %-----------------------------
        % estimates for r^2_{delta} AFW
        fprintf('unbiased r^2_{delta} AFW:\n');

        % percentile
        fprintf('  percentile CI: %8.6g\t%8.6f\t%8.6f\t%8.6g\n',r_squared_c_W*UB_weight_delta,j_r_squared_c_W_lower*UB_weight_delta,j_r_squared_c_W_upper*UB_weight_delta,c_locus_pairs);

        if jack_pct_CI_values(1,14) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_delta_AFW_UB, jack_pct_CI_values(1,13), jack_pct_CI_values(1,14));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_delta_AFW_UB, jack_pct_CI_values(1,13), 'infinity');
        end
        fprintf('\n');
        ...
        % upper_bound_Ne = 1/(3*(r_squared_c_W*UB_weight_delta - jack_norm_CI_values(1,1)*UB_weight_delta));
        % lower_bound_Ne = 1/(3*(r_squared_c_W*UB_weight_delta - jack_norm_CI_values(1,2)*UB_weight_delta));

        if jack_norm_CI_values(1,14) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_delta_AFW_UB, jack_norm_CI_values(1,13), jack_norm_CI_values(1,14));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_delta_AFW_UB, jack_norm_CI_values(1,13), 'infinity');
        end
        fprintf('\n');

        %-----------------------------
        % estimates for r^2_{delta} AFT
        fprintf('unbiased r^2_{delta} AFT:\n');

        % percentile
        fprintf('  percentile CI: %8.6g\t%8.6f\t%8.6f\t%8.6g\n',r_squared_c_TH*UB_weight_delta,j_r_squared_c_TH_lower*UB_weight_delta,j_r_squared_c_TH_upper*UB_weight_delta,c_locus_pairs);

        if jack_pct_CI_values(1,16) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_delta_AFT_UB, jack_pct_CI_values(1,15), jack_pct_CI_values(1,16));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_delta_AFT_UB, jack_pct_CI_values(1,15), 'infinity');
        end
        fprintf('\n');

        % normal distribution
        fprintf('  normal CI:\t%8.6g\t%8.6f\t%8.6f\t%8.6g\n',r_squared_c_TH*UB_weight_delta,r_squared_c_AFT_norm_CI(1,1)*UB_weight_delta,r_squared_c_AFT_norm_CI(1,2)*UB_weight_delta,c_locus_pairs);

        if jack_norm_CI_values(1,16) >= 0 
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%8.6g\n', Ne_r_squared_delta_AFT_UB, jack_norm_CI_values(1,15), jack_norm_CI_values(1,16));
        else
            fprintf('  Ne & CI:\t%8.6g\t%8.6g\t%s\n', Ne_r_squared_delta_AFT_UB, jack_norm_CI_values(1,15), 'infinity');
        end
        fprintf('\n');
    ...
    if make_graphs
        ...
        hold on;
        subplot(2,2,1);
        histogram(j_r_squared_c(:,1)); % 1st column is AFW, 2nd column is AFT
        xlabel('r_c^2 AFW')
        ylabel('Count')

        subplot(2,2,2);
        histogram(j_r_squared_c(:,2)); % 1st column is AFW, 2nd column is AFT
        xlabel('r_c^2 AFT')
        ylabel('Count')

        subplot(2,2,3)
        histogram(j_r_squared_delta(:,1)); % 1st column is AFW, 2nd column is AFT
        xlabel('r_{\Delta}^2 AFW')
        ylabel('Count')

        subplot(2,2,4)
        histogram(j_r_squared_delta(:,2)); % 1st column is AFW, 2nd column is AFT
        xlabel('r_{\Delta}^2 AFT')
        ylabel('Count')

        sgtitle('Distributions of delete-one jackknife r^2 sub-estimates from all pairs of alleles.');
    ...
    if print_output
        ...
        fprintf('delete-one-locus-pair jackknife estimate of variance of r^2 (following Waples and Do 2007\n');
