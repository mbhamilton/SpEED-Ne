...
%   The Statistics Toolbox of Matlab is required. Uses
%   SpEED_Ne.fig. 
%  
%   version 2.1  02 January 2017
%
%**************
...
%     % equations based on Table 1 in Waples and Do (2007)
%     fprintf('\n');
%     fprintf('Ne estimate based on unbaised r^2_{delta} with allele frequency threshold (AFT) \n');
%     fprintf('and harmonic mean sample size of individuals (S),\n');
%     fprintf('using the statistical fit correction of Waples (2006) as implemented in LDNe\n');
%     fprintf('[see Waples & Do (2007) Table 1].\n');
% 
%     if har_mean_S_delta < 30
%         E_r2_delta_sample = 0.0018 + 0.907/har_mean_S_delta + 4.44/(har_mean_S_delta^2);
%         r_2_prime = ub_r_squared_delta_TH - E_r2_delta_sample;
%         waples_Ne = (0.308 + sqrt(0.308^2 + 2.08*r_2_prime^2) )/ (2*r_2_prime);
...
%         r_2_prime = ub_r_squared_delta_TH - E_r2_delta_sample;
%         waples_Ne = (1/3 + sqrt(1/9 + 2.76*r_2_prime^2) )/ (2*r_2_prime);
%         fprintf('Estimate based on  statistical fit for sample size >= 30:\n');
%         fprintf('\tNe = %8.2f\n', waples_Ne);
%     end;
