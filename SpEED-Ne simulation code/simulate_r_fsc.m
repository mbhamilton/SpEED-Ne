...
%
% This version uses simulated data files generated by fastsimcoal in
% arlequin format read with function read_arp.m. 
%
...
%**************
    ...
    files_path = '/Applications/fastsimcoal/fsc_mac64/fsc_msat_u0p025_r0p5_Ne50/';
    
    % output file name or full path - .mat file for all results
    out_file_name = '/Applications/fastsimcoal/fsc_mac64/fsc_msat_u0p025_r0p5_Ne50/fsc_msat_u0p025_r0p5_Ne50.mat';
    ...
    %max_freq_null = 0.10; % when nulls simulated, max frequency of an allele that can be randomly picked as a null allele
...
%             allele_freq_table(num_loci, orig_num_alleles, orig_allele_freqs, orig_allele_states); % print table of allele states and frequencies for all loci
        ...
        % compute and apply Weir N/(N-1) factor for unbaised estimator
        ...
        Ne(iter,8) = 1/(3*(r_squared_delta_UB(iter,2) - r_delta_correction_factor_har_mean_S)); % r_squared_delta AFT UB
            ...
            % version with multiplication by UB_factor(iter,2) to give unbiased estimate of r_squared delta. 
        ...
        fprintf('File %d of %d completed.\n',iter,num_sims);

    end; % for sim=1:num_sims

    save(out_file_name); % save all variables from the current workspace to a .mat file. If filename exists, save overwrites the file.
