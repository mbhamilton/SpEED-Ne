function [output] = generate_pop(mating_model,nloci,k,Ne,gens,write_file,outfilename)
...
%   outfilename - string name of output file if one is written
%
% Outputs
    ...
    for i=1:Ne
        ...
        for locus=1:nloci
            ...
            if rand < 0.5 %determine which allele to sample from parent
                ...
                genotypes(i,2*locus-1,1) = founders(parent_one,2*locus);
            end
