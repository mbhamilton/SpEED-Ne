function estimate_r_sim(genotypes,threshold_allele_freq,nloci,num_individuals)

% Program that calls functions to estimate r_squared_comp and r_squared_delta using simulated genotype data. 
% Utilizes the same underlying estimation functions as the GUI version of SpEED-Ne.
%
% This version uses simulated genotype data based on spatial simulation of clonal plants. 
%
...
%	UB = unbiased because of multiplation by S/(S-1)
%
...
%     sum(counts) % show number of <=zero for all 16 estimators        
    ...
    fprintf('disequilibrium estimates completed.\n');
