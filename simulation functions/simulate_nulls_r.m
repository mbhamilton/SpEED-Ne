function simulate_nulls_r
% Program designed as a simulator that calls functions to estimate
% r_squared_comp and r_squared_delta using simulated data. 
% Utilizes the same underlying functions as the GUI version of SpEED-Ne.
%
% This version simulates genotype data with null alleles via the function sim_null_alleles.m.
% The key variable is max_freq_null. 
%
%   version 1.7  29 Feb 2024
%
...
%
%**************
%   Copyright 2024 by Matthew B Hamilton. 
%
...
%**************
    ...
    output_filename = 'output_file'; % file name for saved workspace
    file_name_ext = '.mat'; % file name extension
    
    pop_size = 10; % true Ne values
    
    num_sims = 250;   % number of simulation iterations for a given true Ne
    
	max_freq_null = 0.05; % max frequency of an allele that can be randomly picked as a null allele
    ...
    mating_model = 2; % random mating w/ possible selfing = 1, random without selfing = 2
    num_loci = 8;
    num_allele_locus = 6;
    gens_2_sim = 7;
    ...
    for iter = 1:num_sims
        ...
        [genotypes] = generate_pop(mating_model,num_loci,num_allele_locus,pop_size,gens_2_sim,false,'null'); % get simulated genotype data
...
%             fprintf('Allele frequencies before nulls:\n');
%             allele_freq_table(num_loci, orig_num_alleles, orig_allele_freqs, orig_allele_states); % print table of allele states and frequencies for all loci
        ...
        % simulate null alleles
        [genotypes] = sim_null_alleles(genotypes,num_individuals,num_loci,max_freq_null,orig_num_alleles,orig_allele_freqs,orig_allele_states);
        [orig_num_alleles,orig_allele_freqs,orig_allele_states] = alleles(genotypes,num_individuals,num_loci); % get allele frequencies and states

%             fprintf('Allele frequencies after nulls:\n');
%             allele_freq_table(num_loci, orig_num_alleles, orig_allele_freqs, orig_allele_states); % print table of allele states and frequencies for all loci
        ...
        median_r_sqr_c_AFT_perm,correction_factor_har_mean_S,genotypes,num_individuals,num_loci,UB_factor(iter,1),UB_factor(iter,2),threshold_allele_freq,alpha,print_output,...
