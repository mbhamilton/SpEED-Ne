function simulate_r_missing_data
...
%
% This version simulates genotype data for each iteration using
% generate_pop.m and then simulates missing genotyope data at random.
% Missing genotype data can missing alleles or missing one locus genotypes.
%  Ne estimates are made for each type of missing data to compare their
%  impacts on Ne estimates. 
%
%   version 1.1  14 July 2017
%
% Makes 16 different estimates of Ne:
%
% Ne(iter,1,page) - r_squared_c AFW
% Ne(iter,2,page) - r_squared_c AFT
% Ne(iter,3,page) - r_squared_c AFW UB
% Ne(iter,4,page) - r_squared_c AFT UB
% Ne(iter,5,page) - r_squared_delta AFW
% Ne(iter,6,page) - r_squared_delta AFT
% Ne(iter,7,page) - r_squared_delta AFW UB
% Ne(iter,8,page) - r_squared_delta AFT UB
% Ne(iter,9,page) - Waples r_squared_delta AFW
% Ne(iter,10,page) - Waples r_squared_delta AFT
% Ne(iter,11,page) - Waples r_squared_delta AFW UB
% Ne(iter,12,page) - Waples r_squared_delta AFT UB
% Ne(iter,13,page) - r_squared_c AFW permute
% Ne(iter,14,page) - r_squared_c AFT permute
% Ne(iter,15,page) - r_squared_c AFW UB permute
% Ne(iter,16,page) - r_squared_c AFT UB permute
%
% where:
...
%
%   iter - iteration of independent data simulation
%
%   page -	1 = full data
%           2 = missing allele version of data
%           3 = missing genotype version of data
%
%**************
...
%**************
    ...
    output_filename = '/Users/matthewhamilton/Dropbox/R project/different missing data sims/test_missing_data.mat'; % file name for saved workspace
    
    pop_size = 25; % true Ne value
    
    num_sims = 25;   % number of simulation iterations for a given true Ne
    ...
    freq_loci_missing = 0.5;
    freq_missing_individs = 0.1;
    
    
    % allocate space
    num_locus_pairs = (num_loci*(num_loci-1))/2;
    
    r_squared_c = zeros(num_sims,2,3); % 1st column is allele frequency weighted (AFW), 2nd column is allele frequency thresholded (AFT), one page for each data set
    r_squared_delta = zeros(num_sims,2,3); % 1st column is allele frequency weighted (AFW), 2nd column is allele frequency thresholded (AFT)
    
%     r_squared_c_values = zeros(1,3);
%     r_squared_delta_values = zeros(1,3);
%     S_c_values = zeros(1,3);
%     S_delta_values = zeros(1,3);
%     c_locus_pairs = zeros(1,3);
%     delta_locus_pairs = zeros(1,3);
    
    r_squared_c_UB = zeros(num_sims,2,3); % unbiased estimates weighted by S/(S-1), 1st column is allele frequency weighted (AFW), 2nd column is allele frequency thresholded (AFT)
    r_squared_delta_UB = zeros(num_sims,2,3); % unbiased estimates weighted by S/(S-1), 1st column is allele frequency weighted (AFW), 2nd column is allele frequency thresholded (AFT)
    
    Ne = zeros(num_sims,16,3);
    UB_factor = ones(num_sims,2,3);  % S/(S-1) weight for unbiased estimate based on Weir(1979). 1st column is r_squared_c, 2nd column is r_squared_delta.
    ...
    for iter = 1:num_sims
        ...
        [miss_allele_genotypes,miss_geno_genotypes] = sim_missing_data(genotypes,num_individuals,num_loci,freq_loci_missing,freq_missing_individs);
...
%         end
        ...
        for data_vers = 1:3
            ...
            elseif data_vers == 2
                genotype_data = miss_geno_genotypes;
            elseif data_vers == 3
                genotype_data = miss_allele_genotypes;
            end
            ...
            [r_squared_c(iter,1),r_squared_c(iter,2),r_squared_delta(iter,1),r_squared_delta(iter,2),r_squared_c_values,r_squared_delta_values,...
                ...
                make_estimate(print_notes,genotype_data,num_individuals,num_loci,print_table,threshold_allele_freq,print_locus_table,make_files);
            ...
            Ne(iter,1,data_vers) = 1/(3*(r_squared_c(iter,1) - correction_factor_har_mean_S)); % r_squared_c AFW
            Ne(iter,2,data_vers) = 1/(3*(r_squared_c(iter,2) - correction_factor_har_mean_S)); % r_squared_c AFT
            Ne(iter,3,data_vers) = 1/(3*(r_squared_c_UB(iter,1) - correction_factor_har_mean_S)); % r_squared_c AFW UB
            Ne(iter,4,data_vers) = 1/(3*(r_squared_c_UB(iter,2) - correction_factor_har_mean_S)); % r_squared_c AFT UB

            Ne(iter,5,data_vers) = 1/(3*(r_squared_delta(iter,1) - r_delta_correction_factor_har_mean_S)); % r_squared_delta AFW
            Ne(iter,6,data_vers) = 1/(3*(r_squared_delta(iter,2) - r_delta_correction_factor_har_mean_S)); % r_squared_delta AFT
            Ne(iter,7,data_vers) = 1/(3*(r_squared_delta_UB(iter,1) - r_delta_correction_factor_har_mean_S)); % r_squared_delta AFW UB
            Ne(iter,8,data_vers) = 1/(3*(r_squared_delta_UB(iter,2) - r_delta_correction_factor_har_mean_S)); % r_squared_delta AFT UB
            ...
            if har_mean_S_delta < 30
                ...
                Ne(iter,9,data_vers) = (0.308 + sqrt(0.308^2 + 2.08*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFW

                r_2_prime = r_squared_delta(iter,2) - E_r2_delta_sample;
                Ne(iter,10,data_vers) = (0.308 + sqrt(0.308^2 + 2.08*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFT               

                r_2_prime = r_squared_delta_UB(iter,1) - E_r2_delta_sample;
                Ne(iter,11,data_vers) = (0.308 + sqrt(0.308^2 + 2.08*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFW UB

                r_2_prime = r_squared_delta_UB(iter,2) - E_r2_delta_sample;
                Ne(iter,12,data_vers) = (0.308 + sqrt(0.308^2 + 2.08*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFT UB

            else % har_mean_S_delta >= 30
                E_r2_delta_sample = 1/har_mean_S_delta + 3.19/(har_mean_S_delta^2);

                r_2_prime = r_squared_delta(iter,1) - E_r2_delta_sample;
                Ne(iter,9,data_vers) = (1/3 + sqrt(1/9 + 2.76*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFW

                r_2_prime = r_squared_delta(iter,2) - E_r2_delta_sample;
                Ne(iter,10,data_vers) = (1/3 + sqrt(1/9 + 2.76*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFT              

                r_2_prime = r_squared_delta_UB(iter,1) - E_r2_delta_sample;
                Ne(iter,11,data_vers) = (1/3 + sqrt(1/9 + 2.76*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFW UB

                r_2_prime = r_squared_delta_UB(iter,2) - E_r2_delta_sample;
                Ne(iter,12,data_vers) = (1/3 + sqrt(1/9 + 2.76*r_2_prime^2) )/ (2*r_2_prime); % Waples r_squared_delta AFT UB
            ...
            Ne(iter,13,data_vers) = 1/(3*(r_squared_c(iter,1) - median_r_sqr_c_AFW_perm)); % r_squared_c AFW permute
            Ne(iter,14,data_vers) = 1/(3*(r_squared_c(iter,2) - median_r_sqr_c_AFT_perm)); % r_squared_c AFT permute

            Ne(iter,15,data_vers) = 1/(3*(r_squared_c_UB(iter,1) - median_r_sqr_c_AFW_perm)); % r_squared_c AFW UB permute
            Ne(iter,16,data_vers) = 1/(3*(r_squared_c_UB(iter,2) - median_r_sqr_c_AFT_perm)); % r_squared_c AFT UB permute


            fprintf('data version %d of 3 completed.\n',data_vers);

        end % for data_vers = 1:3
