function simulate_r_recombination
...
%**************
    ...
    output_filename = '/Users/Wright/Desktop/Maria/test/26_June_test'; % file name for saved workspace
    ...
    r = [0.5, 0.1, 0.5, 0.1, 0.5, 0.1, 0.5, 0.1, 0.5, 0.1, 0.5];


    for iter = 1:num_sims
        
        % get simulated genotype data
        [genotypes] = generate_pop_r(mating_model,r,num_loci,num_allele_locus,pop_size,gens_2_sim,false,'null'); 
        ...
        [Ne(iter,9)] = Waples_Ne_estimate(har_mean_S_delta,r_squared_delta(iter,1)); % Waples Ne from r_squared_delta AFW
            
        [Ne(iter,10)] = Waples_Ne_estimate(har_mean_S_delta,r_squared_delta(iter,2)); % Waples Ne from r_squared_delta AFT
                
        [Ne(iter,11)] = Waples_Ne_estimate(har_mean_S_delta,r_squared_delta_UB(iter,1)); % Waples Ne from r_squared_delta_UB AFW
    
        [Ne(iter,12)] = Waples_Ne_estimate(har_mean_S_delta,r_squared_delta_UB(iter,2)); % Waples Ne from r_squared_delta_UB AFT
